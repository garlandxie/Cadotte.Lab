...
## Retrieving IDs in Genbank Nuccore Database 
...
xml_parse <- function(query, silent = F, retmax = 10) {
  ...
  # if no results are shown for the query
  if (length(Tt$ids) == 0) {
...
# Retrieves bioinformatic information about each individual search query from the respective XML documents 
# (e.g. DNA sequence names, DNA sequence lengths)
# Output is a dataframe that contains bioinformatic information about each query
...
# Final function that combines xml_parse and read.seq 
